...
class View(Tk):
    def __init__(self):
        ...
        self.objTemp = ObjectTemp() #Khởi tạo đối tượng nhiệt độ, độ ẩm
    ...
    def createTabStatistic(self):
        style = Style() # Cài đặt phần hiển thị cho bảng
        style.configure("Treeview.Heading", foreground='black', font=('Arial Bold', 8))
        self.tbl = Treeview(self.tabStatistic) # Khai báo bảng
        self.tbl.pack(fill='both', expand=True)
        self.tbl['columns'] = ('date', "time", "name", "win", 'Tt', 'Tn', "RHt", "RHn", "AHt", "AHn", "Tdpt", "Tdpn", "weather", "solution") # Khai báo các cột trong bảng
        ...
        self.loadDataStatistic() #Load dữ liệu ra bảng
    ...
    def loadDataStatistic(self):
        ...
        for item in data: #Truy xuất dữ liệu và hiển thị ra bảng
            tmpTemp = ObjectTemp() #Khởi tạo đối tượng nhiệt độ
            tmpTemp.setData(item) #Cập nhật dữ liệu cho đối tượng
            dataItem = tmpTemp.getData() #Tính toán dữ liệu nhiệt độ độ ẩm
            id += 1 # Định danh của dữ liệu
            dateObj = datetime.datetime.strptime(item['created_at'], "%Y/%m/%d %H:%M:%S.%f") #Xử lý dữ liệu thời gian
            date = dateObj.strftime("%Y/%m/%d") #Lấy ra Năm/Tháng/Ngày
            time = dateObj.strftime("%H:%M:%S") #Lấy ra Giờ:Phút:Giây
            #Thêm mới dòng vào bảng
    ...
    def callbackBtnResult(self):
        ...
        self.lbResult.insert(END, self.objTemp.getSolution()) # Điền kết quả dựa trên dữ liệu lên màn hình
        self.lbResult.config(state=DISABLED) # Không cho phép sửa nội dung trên ô kết quả

        #Hiển thị thông tin các thông số lên màn hình của Tab Nhập liệu
    ...
    # chức năng lấy dữ liệu từ màn hình
    def getDataView(self):
        # Kiểm tra và lấy dữ liệu thời tiết
        valWeather = ""
        ...
        #Đưa dữ liệu từ dao diện lên chương trình
        self.objTemp.setWeather(valWeather) #Thời tiết
        self.objTemp.setName(self.etFactoryName.get()) #Tên nhà kho
        self.objTemp.setTempIn(self.etTempIn.get()) #Nhiệt độ trong kho
        self.objTemp.setTempOut(self.etTempOut.get()) #Nhiệt độ ngoài kho
        self.objTemp.setHumidityIn(self.etHumidityIn.get()) #Độ ẩm trong kho
        self.objTemp.setHumidityOut(self.etHumidityOut.get()) #Độ ẩm ngoài kho
        self.objTemp.setWin(self.valWinLevel.get()) #Cấp gió


    # Sự kiện ấn vào nút <Lưu>
